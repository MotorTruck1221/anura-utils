...
const output = fs.createWriteStream(path.join(path.dirname(fileURLToPath(import.meta.url)), `${manifest.package}.app.zip`));
...
archive.on("warning", function (err) {
    if (err.code === "ENOENT") { console.warn(pc.yellow(err)); }
    else { throw err }
});
